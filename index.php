...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Get the POST data
    $userName = $_POST['post_user_name'];
    $postContent = $_POST['post_content'];

    
    if (isset($_FILES['post_user_photo']) && $_FILES['post_user_photo']['error'] == 0) {
        $userPhotoDir = 'media/user_photo/';
        $userPhotoName = basename($_FILES['post_user_photo']['name']);
        $userPhoto = $userPhotoDir . $userPhotoName;
        
        if (move_uploaded_file($_FILES['post_user_photo']['tmp_name'], $userPhoto)) {
          
        } else {
            echo "Failed to upload user photo.";
        }
    }

    
    $postPhotos = [];
    if (isset($_FILES['post_photos']) && !empty($_FILES['post_photos']['name'][0])) {
        $postPhotosDir = 'media/posts/';
        foreach ($_FILES['post_photos']['tmp_name'] as $key => $tmpName) {
            $fileName = basename($_FILES['post_photos']['name'][$key]);
            $uploadPath = $postPhotosDir . $fileName;
            
            if (move_uploaded_file($tmpName, $uploadPath)) {
                $postPhotos[] = $uploadPath; 
            }
        }
    }

   
    $postPhotosString = implode(',', $postPhotos);

    
    $postVideo = null;
    if (isset($_FILES['post_video']) && $_FILES['post_video']['error'] == 0) {
        $postVideoDir = 'media/videos/';
        $postVideoName = basename($_FILES['post_video']['name']);
        $postVideo = $postVideoDir . $postVideoName;
        if (move_uploaded_file($_FILES['post_video']['tmp_name'], $postVideo)) {
    ...
    try {
        ...
        $sql = "INSERT INTO posts (user_name, user_photo, post_content, post_photos, post_video) 
                VALUES (:user_name, :user_photo, :post_content, :post_photos, :post_video)";
        ...
        $stmt->bindParam(':post_photos', $postPhotosString);  
        $stmt->bindParam(':post_video', $postVideo);
            ...
            .then(posts => {
                    ...
                    postsHtml += `
                        <div class="user-post">
                            <div class="user-post-header">
                                <div class="post-info">
                                    ...
                                    <div class="user-details">
                                        <a class="author" href="#">${post.user_name}</a>
                            ...
                            <div class="post-media">
                                <img src="${post.post_photos}" alt="" />
                            </div>
                            ${post.post_video ? `<div class="post-media media-video">
                                <video controls>
                                    <source src="${post.post_video}" />
                                </video>
                            </div>` : ''}
                ...
                <div class="comments-menu">
                  <ul>
                    ...
                    <li class="comment_click" data-bs-toggle="modal" data-bs-target="#create_comment_modal" commentId="">
                      <span class="comment-icon"></span>
                      <span>Comment</span>
                    </li>
...
<!--comment area start--> 
  ...
  <div class="comment-item">
    <img class="auth" src="" alt="">
    <div class="comment-data">
      <span></span>
      <p></p>
      
      
        <img class="cphoto" src="" alt="">
      ...
      <div class="comment-item" data-comment-id="" style="border: none;">
        <div class="creply-area">
            
            <span></span>
          <a href=""><span class="comm_like"></span><i id="like_icon" class="fa-solid fa-thumbs-up"></i> Like</a>
          ...
          <div class="tarea" style="display: none;">
            <div class="tarea-in">
              <div class="reply-user">
                ...
                <label  class="replyimg" >
                    
                     <img src="./assets/images/userimg.jpg" alt=""  >
                    
               <input style="display:none" type="file" name="replyphoto" id="file-input">
                
                </label>
                
                <input hidden name="replyid" class="reply-id"> <!-- This hidden input will be set dynamically -->
              </div>
              <textarea placeholder="Type your reply here..." name="textarea"></textarea>
              <input class="comm-post btn btn-primary" type="submit" value="Reply Comment" name="reply_submit">
            </div>
            <p style="color:red;"></p>
          </div>
        </div>
...
<!-- new Reply display area -->
          ...
          <div class="comment-item reply-item">
            ...
            <div class="comment-data">
              ...
              <div class="creply-area" style="display:flex; gap:10px;">
              <p></p>
              <a href="" class="likebtn"><span class="comm_like"></span><i class="fa-solid fa-thumbs-up"></i> Like</a>
             </div>
            </div>
          </div>
        ...
        <div class="creply-area">
          ...
          <div class="tarea" style="display: none;">
            <div class="tarea-in">
              <div class="reply-user">
                <input type="text" name="username" placeholder="Enter Your Name">
                <input name="replyid" class="reply-id"> 
              </div>
              <textarea placeholder="Type your reply here..."></textarea>
              <button class="comm-post btn btn-success" name="reply_submit">Post</button>
            </div>
          </div>
        </div>
      </div> -->
...
<div class="modal fade" id="create_post_modal">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <h2>Create Post</h2>
                <hr>
                <form id="createPostForm" action="your_php_script.php" method="POST" enctype="multipart/form-data">
                    <div class="my-2">
                    ...
                    <div class="my-2">
                        ...
                        <input type="file" class="form-control" name="post_video" placeholder="video">
                    </div>
                    <div class="my-2">
      ...
      <div class="modal-content">
        <div class="modal-body">
          ...
          <form action="" method="POST" enctype="multipart/form-data">
            <div class="my-2">
              <label for="">Auth User Name</label>
              <input type="text" class="form-control" name="comment_user_name" placeholder="Name">
            </div>
            <div class="my-2">
              <label for="">Auth User Photo</label>
              <input type="file" class="form-control" name="comment_user_photo" placeholder="photo">
            </div>
            <div class="my-2">
              <input type="text" class="form-control" name="comment_id" id="comment_id" placeholder="Name">
            </div>
            <div class="my-2">
              <label for="">Comment Content</label>
              <textarea name="comment_content" id="" class="form-control"></textarea>
            </div>

            <div class="my-2">
              <label for="">Comment Photo</label>
              <input type="file" name="comment_photo" class="form-control" placeholder="photo">
            </div>



            <div class="my-2">
              <input type="submit" name="comment_submit" class="btn btn-primary d-block w-100" value="Create Comment">
            </div>
          </form>
        </div>
      </div>
  ...
  document.querySelectorAll('.reply-btn').forEach(button => {
    button.addEventListener('click', function() {
        const tareaDiv = this.nextElementSibling; // Get the reply textarea div
        const commentItem = this.closest('.comment-item'); // Get the closest comment item

        // Ensure that commentItem has a data-comment-id attribute
        const commentId = commentItem ? commentItem.getAttribute('data-comment-id') : null;

        if (commentId) {
            // Toggle display
            tareaDiv.style.display = tareaDiv.style.display === 'none' ? 'block' : 'none';

            // Find the reply input and p tag within tareaDiv
            const replyIdInput = tareaDiv.querySelector('.reply-id');
            

            // Update the hidden input and p tag with the comment ID
            if (replyIdInput) {
 ...
 const commlikeid = document.querySelectorAll(".comm_like");

// Loop through each comm_like span
    commlikeid.forEach(item => {
    // Find the associated thumbs-up icon (it’s the next sibling of the span)
    const likeIcon = item.nextElementSibling;

    // Check if the textContent of the span is greater than 0
    if (item.textContent > 0) {
        likeIcon.style.color = "#0095ff"; // Change the color to blue
    }
});
...
<script>
   document.querySelectorAll('.reply-btn').forEach(button => {
        button.addEventListener('click', () => {
            const tareaDiv = button.nextElementSibling; // Get the next sibling (the textarea div)
            if (tareaDiv.classList.contains('show')) {
                tareaDiv.classList.remove('show'); // Fade out
                setTimeout(() => {
                    tareaDiv.style.display = 'none'; // Set display to none after fade out
                }, 500); // Match timeout with CSS transition duration
            } else {
                tareaDiv.style.display = 'block'; // Set display to block immediately
                // Allow for rendering before adding class for transition
                requestAnimationFrame(() => {
                    tareaDiv.classList.add('show'); // Fade in
                });
            }
        });
    });
...
$(document).ready(function() {
    ...
    $('#createPostForm').on('submit', function(e) {
        ...
        // Send AJAX request
        $.ajax({
            url: '', // Your PHP script to handle the form
            type: 'POST',
            data: formData,
            contentType: false,  // Don't set content type for FormData
            processData: false,  // Don't process data (needed for file upload)
            success: function(response) {
                // Assuming PHP returns a success message
                $('#responseMessage').html('<div class="alert alert-success">Post created successfully!</div>');

                // Reset the form fields after successful submission
                $('#createPostForm')[0].reset();  // Reset form fields
                $('#responseMessage').html('');  // Clear response message
